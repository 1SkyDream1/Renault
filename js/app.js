 ...
 */
    ...
    (() => {
        ...
        const marksMenu = document.querySelector(".marks-menu");
        const mainSection = document.querySelector(".page--home .main");
        if (marksMenu && mainSection) {
            function adjustMargin() {
                if (window.innerWidth > 991.98) {
                    mainSection.style.marginTop = -marksMenu.clientHeight + "px";
                    mainSection.style.paddingTop = marksMenu.clientHeight + "px";
                } else {
        ...
        function setAllRatings() {
            const ratingItems = document.querySelectorAll(".item-statistics");
            ratingItems.forEach((item => {
                const ratingBar = item.querySelector(".item-statistics__rating-bar");
                const ratingValue = item.querySelector(".item-statistics__value");
        ...
        setAllRatings();
        function setCircularRating() {
            const ratingBar = document.querySelector(".result-statistics__circle--2");
            const ratingValue = document.querySelector(".result-statistics__value");
            if (ratingBar && ratingValue) {
                const rating = ratingResult.reduce(((i, j) => i + j), 0) / ratingResult.length;
                const circumference = 2 * Math.PI * 125;
                const offset = circumference - rating / 5 * circumference;
                ratingBar.style.strokeDashoffset = offset;
                ratingValue.textContent = rating.toFixed(2);
            }
        }
        setCircularRating();
        function setAllRatings2() {
            const ratingItems = document.querySelectorAll(".right-reviews-2__review");
            ratingItems.forEach((item => {
                const ratingBar = item.querySelector(".right-reviews-2__bar");
                const ratingValue = item.querySelector(".right-reviews-2__value");
                if (ratingBar && ratingValue) {
                    const rating = parseFloat(ratingValue.textContent);
                    ratingBar.style.width = rating * 20 + "%";
                }
        ...
        setAllRatings2();
        const rightReviews2Rating = document.querySelector(".right-reviews-2__rating");
        const rightReviews2Value = document.querySelectorAll(".right-reviews-2__value");
        const ratingResult2 = [];
        if (rightReviews2Value.length) {
            rightReviews2Value.forEach((item => {
                ratingResult2.push(parseFloat(item.innerHTML));
            }));
            const result2 = rightReviews2Rating.querySelector(".rating__value");
            result2.innerHTML = ratingResult2.reduce(((i, j) => i + j), 0) / ratingResult2.length;
        }
        ...
        function initSliders() {
            buildSliders();
            if (document.querySelector(".slider-reviews-1")) {
                const thumbnailReviews = new swiper_core_Swiper(".thumbs-reviews-1", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 4,
                    ...
                    loop: true,
                    navigation: {
                        prevEl: ".thumbs-reviews-1__arrow--left",
                        nextEl: ".thumbs-reviews-1__arrow--right"
                    },
                    on: {}
                });
                new swiper_core_Swiper(".slider-reviews-1", {
            ...
            if (document.querySelector(".slider-review")) new swiper_core_Swiper(".slider-review", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
                ...
                loop: true,
                navigation: {
                    prevEl: ".marks-review__button--left",
                    nextEl: ".marks-review__button--right"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
        ...
        class ScrollWatcher {
            ...
            getScrollWatcherConfig(paramsWatch) {
                ...
                if (paramsWatch.margin.indexOf("px") < 0 && paramsWatch.margin.indexOf("%") < 0) {
                    this.scrollWatcherLogging(`Эй, настройку data-watch-margin нужно задавать в PX или %`);
                    return;
                }
